...
GEMINI_API_KEY = "AIzaSyBAStVrZHEgwISefaNT0orYpFYUfdK60SE"
...
def process_doc():
    ...
    try:
        ...
        doc_files = [
            "Payables User Guide.docx",
            "India Local User Guide.docx",
            "India Local Implementation.docx"
        ...
        for doc_filename in doc_files:
            file_path = os.path.join(base_dir, doc_filename)
            
            # Skip files that don't exist
            if not os.path.exists(file_path):
                st.warning(f"File not found: {doc_filename}")
                continue
            ...
            try:
                ...
                file_text = f"DOCUMENT: {doc_filename}\n{text}\nEND OF DOCUMENT: {doc_filename}\n\n"
                all_text += file_text
        ...
        if not file_summaries:
            st.error("No DOCX files could be processed. Please check the file paths.")
            return False
        ...
        st.error(f"Error processing DOCX files: {e}")
        return False
...
4. Web scraping for specific details (e.g., table columns from Oracle ETRM).

When answering questions:
- If the question is about SQL, generate a valid SQL query for the requested task.
- If the question is about Oracle Payables or Indian Localization, use the provided documents and web content to generate a comprehensive response.
...
- Generate a valid SQL query for the requested task.
    ...
    if not is_authenticated():
        ...
        if st.session_state["doc_data"] is not None:
            ...
            with col2:
                ...
                if st.session_state.doc_data and 'files' in st.session_state.doc_data and len(st.session_state.doc_data['files']) > 1:
                    doc_options = ["All Documents"] + [file_info['filename'] for file_info in st.session_state.doc_data['files']]
                    selected_doc = st.selectbox("Filter by:", doc_options, label_visibility="collapsed")
                    st.session_state.selected_doc = selected_doc if selected_doc != "All Documents" else None
                else:
            ...
            if query:
                ...
                # Append document filter to query if selected
                display_query = query
                if hasattr(st.session_state, 'selected_doc') and st.session_state.selected_doc:
                    display_query = f"{query} (filtered to {st.session_state.selected_doc})"
                ...
                with st.spinner("Analyzing documentation and web sources..."):
                    ...
                    if schema_name and table_name:
                        ...
                        # Filter chunks by selected document if applicable
                        if hasattr(st.session_state, 'selected_doc') and st.session_state.selected_doc:
                            filtered_chunks = [chunk for chunk in doc_chunks if f"DOCUMENT: {st.session_state.selected_doc}" in chunk]
                            # If we found filtered chunks, use them, otherwise fall back to all chunks
                            if filtered_chunks:
